...
def supervised_tabular_missing_main(cfg: DictConfig) -> None:
    ...
    for test_fold, val_fold, train, val, test, last_val_fold in cv.get_cross_validation(cfg.paths.cv, "train", "val", "test"):
        ...
        for train_missing_fraction in cfg.missing_percentages:
            ...
            train_set = instantiate(cfg.db.dataset_class, train_data_missing, train_labels, cfg.db.task, "train", preprocessing_params=dataset.preprocessing_params, preprocessing_paths=preprocessing_paths, test_fold=test_fold, val_fold=val_fold, augmentation=cfg.model_name.startswith(("naim", "survival_naim")), normalize_target=cfg.db.get("normalize_target", False), decimals=cfg.db.get("decimals", 2))
            val_set = instantiate(cfg.db.dataset_class, val_data_missing, val_labels, cfg.db.task, "val", preprocessing_params=dataset.preprocessing_params, preprocessing_paths=preprocessing_paths, test_fold=test_fold, val_fold=val_fold, normalize_target=cfg.db.get("normalize_target", False), decimals=cfg.db.get("decimals", 2))
            ...
            for test_missing_fraction in cfg.missing_percentages:
                ...
                test_set = instantiate(cfg.db.dataset_class, test_data_missing, test_labels, cfg.db.task, "test", preprocessing_params=dataset.preprocessing_params, preprocessing_paths=preprocessing_paths, test_fold=test_fold, val_fold=val_fold, normalize_target=cfg.db.get("normalize_target", False), decimals=cfg.db.get("decimals", 2))
    ...
    metrics.compute_missing_performance(dataset.classes, cfg.paths.predictions, cfg.paths.results, cfg.db.task, performance_metrics, cfg.missing_percentages, decimals=max(cfg.db.get("decimals", 2), 2))
