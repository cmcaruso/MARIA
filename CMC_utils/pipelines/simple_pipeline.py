...
from CMC_utils import metrics, miscellaneous
...
def supervised_learning_main(cfg: DictConfig) -> None:
    ...
    for test_fold, val_fold, train, val, test, last_val_fold in cv.get_cross_validation(cfg.paths.cv, "train", "val", "test"):
        ...
        train_set = instantiate(cfg.db.dataset_class, train_data, train_labels, cfg.db.task, "train", preprocessing_params=dataset.preprocessing_params, preprocessing_paths=cfg.paths.preprocessing, test_fold=test_fold, val_fold=val_fold, augmentation=cfg.model_name.startswith(("naim", "survival_naim")), normalize_target=cfg.db.get("normalize_target", False), decimals=cfg.db.get("decimals", 2))
        val_set = instantiate(cfg.db.dataset_class, val_data, val_labels, cfg.db.task, "val", preprocessing_params=dataset.preprocessing_params, preprocessing_paths=cfg.paths.preprocessing, test_fold=test_fold, val_fold=val_fold, normalize_target=cfg.db.get("normalize_target", False), decimals=cfg.db.get("decimals", 2))
        ...
        test_set = instantiate(cfg.db.dataset_class, test_data, test_labels, cfg.db.task, "test", preprocessing_params=dataset.preprocessing_params, preprocessing_paths=cfg.paths.preprocessing, test_fold=test_fold, val_fold=val_fold, normalize_target=cfg.db.get("normalize_target", False), decimals=cfg.db.get("decimals", 2))
