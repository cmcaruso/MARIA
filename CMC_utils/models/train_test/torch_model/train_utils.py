...
class ModelCheckpointer:
    ...
    def __init__(self, model, optimizer, best_epoch: int = -1, best_loss: float = None, best_performance: list = None ):
        ...
        self.best_optimizer_weights = optimizer.state_dict() #.copy()
...
class CustomScheduler:
    ...
    """
    def __init__(self, learning_rates: list, patience: int, verbose: int, min_epochs: int, epochs_no_improve: int = 0, learning_rate_idx: int = 0, idx_reverse_order: bool = False):
        ...
        if epoch > self.min_epochs:
            ...
            if self.next_step:
                if not self.state["idx_reverse_order"]:
                    ...
                    self.state["learning_rate_idx"] -= 1
                    self.state["idx_reverse_order"] = True if self.state["learning_rate_idx"] > 1 else False
...
class TrainManager:
    ...
    def load_checkpoint(self, model: torch.nn.Module) -> torch.nn.Module:
            ...
            if os.path.exists(checkpoint_path + "_checkpoint.pkl"):
                ...
                optimizer_state_dict = torch.load(checkpoint_path + "_optimizer.pth", map_location=self.device)
