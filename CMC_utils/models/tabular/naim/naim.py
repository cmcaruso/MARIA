...
class Attention(torch.nn.Module):
    ...
    def forward(self, q: Tensor, k: Tensor, attn_mask: Optional[Tensor] = None, attn_mask_2: Optional[Tensor] = None) -> Tensor:
...
class MultiHeadAttention(torch.nn.Module):
    ...
    def _scaled_dot_product_attention(self, q: Tensor, k: Tensor, v: Tensor, attn_mask: Optional[Tensor] = None, attn_mask_2: Optional[Tensor] = None) -> Tuple[Tensor, Tensor]:
        ...
        attn = self.attn(q, k, attn_mask, attn_mask_2)
        """B, Nt, E = q.shape
        q = q / math.sqrt(E)
    ...
    def __init__(self,
        ...
        if not self.extractor:
            if self.output_size > 1:
                self.classifier = torch.nn.Sequential(torch.nn.Linear(self.embed_size, self.output_size))
            else:
                if limit_regression_output:
                    self.classifier = torch.nn.Sequential(torch.nn.Linear(self.embed_size, self.output_size), Sigmoid())
                else:
                    self.classifier = torch.nn.Sequential(torch.nn.Linear(self.embed_size, self.output_size))
    ...
    def forward(self, x, original_x=None):
        if self.embed_input:
            ...
            if self.embed_vector_fun == "clstoken":
                cls_tokens = self.cls_token.repeat(embeddings.shape[0], 1, 1).to(embeddings.device)
                embeddings = torch.cat((cls_tokens, embeddings), dim=1)
                masks, masks2 = self.attention_mask.modality_mask(embeddings, original_x, use_cls_token=True)
            elif self.embed_vector_fun == "mean":
                masks, masks2 = self.attention_mask.modality_mask(embeddings, original_x)
            else:
                original_x = torch.cat(original_x, dim=1)
                masks, masks2 = self.attention_mask.mask(original_x)
